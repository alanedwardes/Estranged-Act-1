...
int CWorldLights::GetWorldLightsSize()
{
	return m_nWorldLights;
}

dworldlight_t* CWorldLights::GetWorldLights()
{
	return m_pWorldLights;
}
...
int __cdecl SortLightsByDistance( const d_ae_shaderlight_t *pLeft, const d_ae_shaderlight_t *pRight )
{
	//Get player eye position
	Vector vecPlayer = C_BasePlayer::GetLocalPlayer()->EyePosition();

	//Setup left vector
	Vector vecLeft;
	vecLeft.Init(pLeft->pos[0], pLeft->pos[1], pLeft->pos[2]);

	//Setup right vector
	Vector vecRight;
	vecRight.Init(pRight->pos[0], pRight->pos[1], pRight->pos[2]);

	//Get distances
	vec_t distance1 = (vecPlayer - vecLeft).LengthSqr();
	vec_t distance2 = (vecPlayer - vecRight).LengthSqr();
...
// -------------------------------------------
// Purpose: Update the Shader API interface, passing it updated light information
// Author: AniCator
// Based of: CWorldLights::GetBrightestLightSource
// -------------------------------------------
void CWorldLights::UpdateShaderLights()
{
	if(!m_nWorldLights || !m_pWorldLights)
		return;

	Vector vecPosition = C_BasePlayer::GetLocalPlayer()->EyePosition();
	...
	//Test for ShaderAPI interface - AniCator
	GetShaderAPIExt()->CommitWorldLightsSize(m_nWorldLights);
	CUtlVector<d_ae_shaderlight_t> lightList;

	// Iterate through all the world lights
	for(int i = 0; i < m_nWorldLights; ++i)
	{
		dworldlight_t *light = &m_pWorldLights[i];
		...
		// Calculate square distance to this world light
		/*Vector vecDelta = light->origin - vecPosition;
		float flDistSqr = vecDelta.LengthSqr();
		float flRadiusSqr = light->radius * light->radius;
		...
		// Check if player can see light
		/*Vector vecAbsStart = vecPosition + Vector(0,0,30);
		Vector vecAbsEnd = light->origin;

		trace_t tr;
		UTIL_TraceLine(vecPosition, vecAbsEnd, CONTENTS_SOLID, NULL, COLLISION_GROUP_NONE, &tr);
		...
		d_ae_shaderlight_t shaderlight;
		shaderlight.pos[0] = light->origin[0];
		shaderlight.pos[1] = light->origin.y;
		shaderlight.pos[2] = light->origin.z;
		shaderlight.color[0] = light->intensity[0];
		shaderlight.color[1] = light->intensity.y;
		shaderlight.color[2] = light->intensity.z;

		lightList.AddToTail(shaderlight);
	}

	//Run a quicksort using SortLightsByDistance function

	/*
	//Debug light list
	DevMsg("LightList Pre-Sort\nPlayer position: %.2f,%.2f,%.2f\n",vecPosition.x,vecPosition.y,vecPosition.z);
	for(int i = 0; i<lightList.Count();i++)
	{
		Vector vecLeft;
		vecLeft.Init(lightList[i].pos[0], lightList[i].pos[1], lightList[i].pos[2]);
		vec_t distance1 = (vecPosition - vecLeft).Length();
		DevMsg("Light %i | dist2player: %.2f | pos[0]:%.2f, pos[1]:%.2f, pos[2]:%.2f\n", i, distance1, lightList[i].pos[0],lightList[i].pos[1],lightList[i].pos[2]);
	}
	DevMsg("--LightList Pre-Sort\n");*/

	lightList.Sort( SortLightsByDistance );

	/*
	//Debug light list
	DevMsg("LightList Post-Sort\nPlayer position: %.2f,%.2f,%.2f\n",vecPosition.x,vecPosition.y,vecPosition.z);
	for(int i = 0; i<lightList.Count();i++)
	{
		Vector vecLeft;
		vecLeft.Init(lightList[i].pos[0], lightList[i].pos[1], lightList[i].pos[2]);
	...
	GetShaderAPIExt()->SetLights( lightList );
}

// -------------------------------------------
// Purpose: Print the amount of lights that are inside the player's PVS
// Author: AniCator
// Based of: CWorldLights::GetBrightestLightSource
// -------------------------------------------
void PrintLightsInPVS( const CCommand &args )
{
	int m_nWorldLights = g_pWorldLights->GetWorldLightsSize();
	dworldlight_t* m_pWorldLights = g_pWorldLights->GetWorldLights();

	if(!m_nWorldLights || !m_pWorldLights)
	{
		DevMsg("m_nWorldLights is 0! Grab your legs and break them!!\n");
		return;
	}
	...
	Vector vecPosition;
	vecPosition = player->EyePosition();
	...
	for(int i = 0; i < m_nWorldLights; ++i)
	{
		...
		// Do not add emit_skyambient to result
		if(light->type == emit_skyambient)
		{
			continue;
		}

		// Do not add emit_skylight to result
		if(light->type == emit_skylight)
		{
			// Calculate sun position
			Vector vecAbsStart = vecPosition + Vector(0,0,30);
	...
	DevMsg("Worldlights found: %i (total amount existing: %i)\n", debugCount, m_nWorldLights);
}

ConCommand my_say( "ae_debug_print_worldlights", PrintLightsInPVS , "Print amount of world lights found in PVS", 0);